						...
						ktime_t now)
{
	...
	if (health != POWER_SUPPLY_HEALTH_UNKNOWN) {
		pr_notice("%s[%d degiC]\n", (chip->batt_hot)?"hot":(chip->batt_cold)?"cold":(chip->batt_warm)?"warm":"cool",
...
static void set_resume_soc_work(struct work_struct *work)
{
	...
	if (is_input_present(chip) && !chip->resume_soc_lowered) {
		...
		if (resume_soc_raw > 0 && resume_soc_raw < FULL_SOC_RAW) {
			...
			if (fg_debug_mask & FG_STATUS) {
				pr_notice("resume soc lowered to 0x%02x\n",
						resume_soc_raw);
			}
...
wait:
	...
	if (!batt_node) {
		pr_notice("No available batterydata, using OTP defaults\n");
		rc = 0;
		goto no_profile;
	}
