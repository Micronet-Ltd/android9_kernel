...
static void virt_gpio_fix_out_set(struct gpio_chip *chip, unsigned offset, int value){
	...
	DEFINE_LOCK_FLAGS(flags); // make last
    LOCK_BANK(dev->gpo_bank.lock, flags);
    //pr_notice("offset %d value %d\n", offset, value);//Michael Efimov: added for test
    switch (offset) {
    case 0:
        if (value) {
            tx_cmd = 0x21;
...
static int __init virtual_gpio_init(void)
{
    ...
    //****************************************************//
	//dev->gpiochip_out.set = virt_gpio_out_set;//Michael Efimov:  comment - it was
	//dev->gpiochip_out.get = virt_gpio_out_get;//Michael Efimov:  comment - it was
